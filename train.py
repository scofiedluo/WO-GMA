...
class train_epoch():
    ...
    """
    def __init__(self,args,model,global_step):
        super().__init__()
        self.model = model
    ...
    def CASLoss(self,features, scores, labels, num_similar,device='cuda'):
        """
    ...
    def train_epoch(self, stage, optimizer, data_loader,experiment,epoch):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            optimizer.zero_grad()
            with torch.no_grad():
                data = data.float().to(device)   # ? for data paralall, this will be modified
            ...
            # if self.global_step<100 or self.global_step%100<20:
            #     # TODO: train TPG
            #     pass
            # else:
            #     # TODO: train OAR
            ...
            # print(torch.sum(proposals))
            # print("TPG_output['video_level_score'] = ",TPG_output["video_level_score"])
            # print("labels = ",label)
            # print(proposals)
            if stage == 'TPG':
                casLoss = self.CASLoss(TPG_output["preprocessed_feature"],TPG_output["TPG_out_scores"],label,self.args.batch_size/2,device)
                if label.shape[1]==1:
                    binary = True
                else:
                    binary = False
                ...
                plot_score(TPG_output["TPG_out_scores"].detach().numpy(), video_names, self.args.action_classes, self.args.work_dir, experiment,epoch,type='TPG_scores')
            elif stage == 'OAR_full':
                self.model.OAR.state = detach(self.model.OAR.state)
                OAR_output = self.model.inference_forward(TPG_output["preprocessed_feature"])
