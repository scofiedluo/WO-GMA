...
class UnfoldTemporalWindows(nn.Module):
    """
    Input shape: (N,C,T,V), out: (N,C,[T/window_stride],V*window_size), where [] means rounded up
    """
    ...
    def forward(self, x):
        # Input shape: (N,C,T,V), out: (N,C,[T/window_stride],V*window_size)
        N, C, T, V = x.shape
        x = self.unfold(x)
...
class SpatialTemporal_MS_GCN(nn.Module):
    ...
    def forward(self, x):
        N, C, T, V = x.shape    # T = number of windows, V = window_size * nodes_num(18)

        # Build graphs
        # A = (V*k,V), where V = window_size * nodes_num(18), and k is the scales num
        A = self.A_scales.to(x.dtype).to(x.device) + self.A_res.to(x.dtype).to(x.device)

        # Perform Graph Convolution
        res = self.residual(x)
        agg = torch.einsum('vu,nctu->nctv', A, x)  # tensor product
